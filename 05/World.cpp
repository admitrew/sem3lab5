...
World::World(const std::string& worldFilePath) {
    ...
    //// Определяем параметры другим способом
    Point center;
    Point velocity;
    double radius;
    Color color;
    ...
    while (stream.peek(), stream.good()) {
        ...
        Ball ball(center, velocity, color, radius, isCollidable); //// Доработка
...
void World::show(Painter& painter) const {
    ...
    for (const Dust& dust : dusts) { //// Добавили обработку частиц
        if (dust.isAlive())
            dust.draw(painter);
    }
}
...
void World::update(double time) {
    ...
    for (Dust& dust : dusts) dust.update(); //// Обработка частиц
